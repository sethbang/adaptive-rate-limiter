...
| `rpm_limit` | `Optional[int]` | `None` | Requests per minute limit |
| `rpm_reset` | `Optional[float]` | `None` | Absolute Unix timestamp (seconds) when RPM resets |
| `tpm_remaining` | `Optional[int]` | `None` | Remaining tokens per minute |
| `tpm_limit` | `Optional[int]` | `None` | Tokens per minute limit |
| `tpm_reset` | `Optional[float]` | `None` | Absolute Unix timestamp (seconds) when TPM resets |
...
class MyProvider(ProviderInterface):
    ...
    async def discover_limits(
        ...
        # If the API has no discovery endpoint, return {} rather than raising.
        # Raise ProviderError (not BackendConnectionError) if the discovery call
        # itself fails — e.g. the endpoint returns 4xx/5xx.
        return {
    ...
    def parse_rate_limit_response(
        ...
        """
        # Coerce through float(): some APIs send integral values
        # float-formatted ("499.0"), which a bare int() rejects.
        #
        # Return None for an absent header rather than a default. Every
        # RateLimitInfo field is Optional precisely so that "the server did
        # not say" stays distinguishable from a real value all the way down
        # to the backend, which syncs the request and token dimensions
        # independently and leaves an unreported one untouched. Substituting
        # a number here fabricates a window the server never described, and
        # the backend will then mark it verified.
        ...
        return RateLimitInfo(
            ...
            # Absolute Unix seconds. If your API sends epoch milliseconds,
            # divide by 1000; if it sends "seconds until reset", add time.time().
            rpm_reset=float(headers["x-ratelimit-reset-requests"])
            if "x-ratelimit-reset-requests" in headers
            else None,
            # A provider that meters requests but not tokens simply leaves
            # these None; the request dimension still syncs normally.
            tpm_remaining=as_int("x-ratelimit-remaining-tokens"),
            tpm_limit=as_int("x-ratelimit-limit-tokens"),
            is_rate_limited=(status_code == 429),
            retry_after=as_int("retry-after") if status_code == 429 else None,
        )
...
When a provider method cannot complete — for example, an upstream discovery API returns a 4xx or 5xx response — raise `ProviderError` (imported from `adaptive_rate_limiter`):

```python
from adaptive_rate_limiter import ProviderInterface, DiscoveredBucket, ProviderError

class MyProvider(ProviderInterface):
    async def discover_limits(
        self, force_refresh: bool = False, timeout: float = 30.0
    ) -> dict[str, DiscoveredBucket]:
        resp = await self._http_client.get("/rate-limits", timeout=timeout)
        if resp.status_code >= 400:
            raise ProviderError(
                f"discover_limits failed: HTTP {resp.status_code}"
            )
        return self._parse_response(resp.json())
```

<Note>
**`ProviderError` vs `BackendConnectionError`**: raise `ProviderError` from `discover_limits`, `parse_rate_limit_response`, and `get_bucket_for_model` when the provider itself fails. `BackendConnectionError` is reserved for storage-backend failures (Redis, in-memory) — do not raise it from provider code.

If your API simply has no discovery endpoint, return `{}` from `discover_limits` rather than raising.
</Note>
...
class MyProvider(ProviderInterface):
    ...
    def parse_rate_limit_response(self, headers, body=None, status_code=None) -> RateLimitInfo:
        ...
        return RateLimitInfo(
            # Leave the field None when the header is absent -- do not
            # substitute a default. See the note on as_int() above.
            rpm_remaining=int(float(raw)) if raw is not None else None,
            is_rate_limited=(status_code == 429),
        )
...
<Note>
**Who normalizes reset headers?** The backend does, on the live feedback path.
The scheduler calls `discover_limits()` and `get_bucket_for_model()`, but it does
**not** call `parse_rate_limit_response()` — response headers are handed to
`Backend.update_rate_limits(headers=...)` and parsed by
`BaseBackend._parse_rate_limit_headers`, which accepts epoch milliseconds, epoch
seconds or a relative delta and normalizes all three to absolute Unix seconds.

Implement `parse_rate_limit_response()` for your own use (inspection, logging,
tests, or a custom backend that calls it), but don't rely on it to change what
the limiter ingests.
</Note>
...
class OpenAIProvider(ProviderInterface):
    ...
    def parse_rate_limit_response(
        ...
        # None for an absent header: a limit of 0 is a real, meaningful value
        # ("no capacity on this dimension"), so it must not double as the
        # placeholder for a header the server never sent.
        def as_int(name: str) -> Optional[int]:
