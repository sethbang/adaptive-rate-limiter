...
| `max_token_delta` | `120` | Largest token-reset window (in seconds) that will be adopted from response headers. A longer window is **not** clamped into range — that would make the scheduler believe token capacity refills earlier than it does and over-send. Instead the window is left unadopted (with a one-time warning) while the observed token counts are still applied. Raise this if your provider's token window genuinely exceeds it. |
...
| `failure_threshold` | `None` | Consecutive-window failure count that opens the circuit breaker. `None` uses `DEFAULT_FAILURE_THRESHOLD` (20). |
| `failure_window_seconds` | `None` | Rolling window, in seconds, over which failures are counted. `None` selects `CLUSTER_FAILURE_WINDOW` (120) in cluster mode and `DEFAULT_FAILURE_WINDOW` (30) otherwise. The window must be **longer than the worst-case connect time**, or failures age out faster than a single failed connect can accumulate them and the breaker can never trip; a cluster connect retries `CLUSTER_PING_ATTEMPTS` times and can span ~45s, which is why cluster mode defaults higher. A warning is logged if you set a cluster window shorter than that budget. |
...
`RedisBackend` has explicit `start()` and `stop()` methods. When used through `StateManager` or `create_scheduler` (the normal path), the scheduler calls them automatically. If you construct a `RedisBackend` directly outside a scheduler, call them yourself:

```python
from adaptive_rate_limiter.backends import RedisBackend

backend = RedisBackend(redis_url="redis://localhost:6379", account_id="default")

# start() connects to Redis AND begins background orphan recovery
await backend.start()
...
try:
    ...
    # stop() halts orphan recovery and cleans up connections
    await backend.stop()
```

### Context Manager Pattern (Recommended)

The `async with` pattern is the recommended alternative: it calls `start()` on entry and `stop()` on exit, including on exceptions.
...
async with backend as rb:
    ...
    # ... use backend for rate limiting ...
    pass
# Auto-cleanup on exit (even if an exception was raised)
```
...
| [`distributed_update_rate_limits`](https://github.com/sethbang/adaptive-rate-limiter/blob/main/src/adaptive_rate_limiter/backends/lua/distributed_update_rate_limits.lua) | Sync state from 2xx response headers. Accepts server-reported limit values verbatim — if the server reports a lower limit than stored (e.g. a tier downgrade), the local limit updates immediately. |
