...
└── RateLimiterError                    # Base exception for all rate limiter errors
    ...
    ├── ProviderError                  # Provider discover/parse failures
...
Raised by custom `ProviderInterface` implementations when `discover_limits`, `parse_rate_limit_response`, or `get_bucket_for_model` cannot complete — for example, when an upstream discovery API returns a 4xx or 5xx response.

<Note>
`ProviderError` is distinct from `BackendConnectionError`, which is reserved for storage-backend failures (Redis, in-memory). Use `ProviderError` in provider code; use `BackendConnectionError` for backend infrastructure issues.
</Note>

**Usage (in a custom provider):**
```python
from adaptive_rate_limiter import ProviderError

class MyProvider(ProviderInterface):
    async def discover_limits(
        self, force_refresh: bool = False, timeout: float = 30.0
    ) -> dict[str, DiscoveredBucket]:
        resp = await self._client.get("/limits", timeout=timeout)
        if resp.status_code >= 400:
            raise ProviderError(
                f"discover_limits failed: HTTP {resp.status_code}"
            )
        return self._parse_discovery_response(resp.json())
```
...
try:
    ...
    # Check provider configuration or upstream API availability
    raise
```
