...
Stale and abandoned stream cleanup is handled automatically by intelligent mode's internal cleanup manager — there is no class to import or instantiate. When a stream has been inactive beyond the configured timeout (default: 5 minutes via `StateConfig.reservation_ttl`), the scheduler reclaims its reservation and returns the capacity to the pool. This prevents capacity leaks when streams fail unexpectedly or are abandoned.
...
Task cancellation mid-stream is safe. `RateLimitedAsyncIterator.__anext__` catches both `asyncio.CancelledError` and `GeneratorExit`, releases the reservation using the conservative zero-refund fallback, and re-raises the exception. This means wrapping a streaming call in `asyncio.wait_for()` or `asyncio.timeout()` will not leak capacity — the reservation is always released even if the task is cancelled before the stream finishes.
...
- Note that cancellations and exceptions now release capacity automatically (with zero refund); only an early `break` without calling `aclose()` can leak — let the stream complete or call `aclose()` on early break

### Capacity Leaks

If capacity isn't being released:
- The scheduler's internal cleanup automatically releases stale streams after the configured timeout (default: 5 minutes via `StateConfig.reservation_ttl`)
