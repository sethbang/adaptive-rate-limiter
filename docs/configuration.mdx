...
The `RateLimiterConfig` class controls the core behavior of the scheduler. It has 29 configurable fields.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `max_retries` | `int` | `3` | Maximum retry attempts for a failed request (BASIC mode). |
| `batch_size` | `int` | `50` | Number of eligible queues processed per scheduler iteration (INTELLIGENT mode). |
| `stale_entry_ttl` | `float \| None` | `None` | TTL in seconds for per-model request-tracking entries (BASIC mode). `None` disables age-based cleanup of tracking state. |
| `max_tracking_entries` | `int \| None` | `None` | Maximum number of per-model request-tracking entries to retain (BASIC mode). `None` means unbounded. |
...
| `scheduler_interval` | Must be `> 0` |
| `backoff_base` | Must be `> 1.0` |
| `max_backoff` | Must be `> 0` |
| `failure_window` | Must be `> 0` |
| `max_failures` | Must be `>= 1` |
| `health_check_interval` | Must be `> 0` |
| `max_consecutive_failures` | Must be `>= 1` |
| `metrics_export_interval` | Must be `> 0` |
| `prometheus_port` | Must be `1–65535` |
| `test_rate_multiplier` | Must be `> 0` |
...
# Invalid: backoff_base must be > 1.0 (1.0 itself is not allowed)
config = RateLimiterConfig(
    backoff_base=1.0,               # ERROR: must be > 1.0
)

# Invalid: prometheus_port must be a valid port number (1–65535)
config = RateLimiterConfig(
    prometheus_port=0,              # ERROR: must be between 1 and 65535
)
```
