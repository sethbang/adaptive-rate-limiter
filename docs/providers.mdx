...
class MyProvider(ProviderInterface):
    ...
    def parse_rate_limit_response(
        ...
        return RateLimitInfo(
            rpm_remaining=as_int("x-ratelimit-remaining-requests", 100),
            rpm_limit=as_int("x-ratelimit-limit-requests", 100),
            ...
            tpm_remaining=as_int("x-ratelimit-remaining-tokens", 10000),
            tpm_limit=as_int("x-ratelimit-limit-tokens", 10000),
            is_rate_limited=(status_code == 429),
            retry_after=as_int("retry-after", 0) if status_code == 429 else None,
        )
...
class MyProvider(ProviderInterface):
    ...
    def parse_rate_limit_response(self, headers, body=None, status_code=None) -> RateLimitInfo:
        return RateLimitInfo(
            rpm_remaining=int(float(headers.get("x-ratelimit-remaining-requests", 100))),
            is_rate_limited=(status_code == 429),
        )
